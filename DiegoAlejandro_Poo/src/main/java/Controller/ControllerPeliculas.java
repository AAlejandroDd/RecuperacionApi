...
import Exceptions.ExceptionActoresNotFound;
import Exceptions.ExceptionColumnDuplicate;
import Models.DTOPeliculas;
import Services.ServicesPeliculas;
...
public class ControllerPeliculas {
    ...
    @Autowired
    private ServicesPeliculas service;

    @GetMapping("/getAllPeliculas")
    private ResponseEntity<Page<DTOPeliculas>> getDataProducts(
        ...
        Page<DTOPeliculas> actores = service.getAllPeliculas(page, size);
        if (actores == null){
            ResponseEntity.badRequest().body(Map.of(
    ...
    private ResponseEntity<Map<String, Object>> inserCategory(@Valid @RequestBody DTOPeliculas json, HttpServletRequest request){
        try{
            DTOPeliculas response =service.insert(json);
            if (response == null){
    ...
    public ResponseEntity<?> modificarUsuario(
            ...
            @Valid @RequestBody DTOPeliculas usuario,
        ...
        try{
            DTOPeliculas usuarioActualizado = service.update(id, usuario);
            return ResponseEntity.ok(usuarioActualizado);
        }
        catch (ExceptionActoresNotFound e){
